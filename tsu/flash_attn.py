...
def flash_attn_qkv_forward_kernel(
    ...
    q = tl.load(q_bp, boundary_check=(0, 1, 2), padding_option='zero')  # [bh, q, d]
    ...
    for _ in tl.range(0, KV, BLOCK_KV):
        k = tl.load(k_bp, boundary_check=(0, 1, 2), padding_option='zero')  # [bh, d, kv]
        v = tl.load(v_bp, boundary_check=(0, 1, 2), padding_option='zero')  # [bh, kv, d]
        ...
        k_bp = tl.advance(k_bp, offsets=(0, 0, BLOCK_KV))
        v_bp = tl.advance(v_bp, offsets=(0, BLOCK_KV, 0))
        kv_index += BLOCK_KV
    ...
    tl.store(l_bp, (tl.log(l) + m).to(dtype=l_ptr.dtype.element_ty), boundary_check=(0, 1))
    tl.store(o_bp, (o / l[:, :, None]).to(dtype=o_ptr.dtype.element_ty), boundary_check=(0, 1, 2))


def flash_attn():
    q = torch.randn((2 * 3, 4, 35)).cuda()
    k = torch.randn((2 * 3, 4, 35)).cuda()
    v = torch.randn((2 * 3, 4, 35)).cuda()
    o = torch.randn((2 * 3, 4, 35)).cuda()
    l = torch.randn((2 * 3, 4)).cuda()

    BLOCK = 32

    flash_attn_qkv_forward_kernel[(q.size(0) + BLOCK - 1) // BLOCK, (q.size(1) + BLOCK - 1) // BLOCK](
        ...
        BLOCK_BH=8, BLOCK_Q=16, BLOCK_KV=16, BLOCK_D=max(16, triton.next_power_of_2(q.size(-1))),
    )

    expected = (q @ k.transpose(-1, -2)).mul(q.size(-1) ** -0.5).softmax(dim=-1) @ v

    print(torch.testing.assert_close(actual=o, expected=expected))
